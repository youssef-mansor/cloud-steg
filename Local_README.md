...
| Endpoint   | Method | Leader Only | Description                 Okay so this is admin UI I want another UI for the user to use 
first the user register or logs in using only his or her user name and then he can see his 128 by 128 images that is both is saved locally and on the server and he can upload more to the server 
then in another tab the user can see who is online when the user can expand the view so each user below it is the 128 by 128 images that he uploaded or the user images if collapsed only the clicked user explands then the user can click on an image a click request to view
then the image request is created then in another tab the user can see the request the he have on his images and when he the request is approved the user must give view count and cover image where everything is encrypted and sent to other user
also the recieved request has the name of the 128 by 128 image and maybe the image itself so the client backend can use to match it with the locally saved image to be sent
when a user opens the or loges in it must start a heartbeat then when the UI is closed stop it
then we need another tab for images that the user can click to view
futurely the server will have add notes and get notes that will be used to update the view count incase the user changed his mind however get notes will be called by the client by the server when the user first becomes online add notes will be used in case the client updated the view count but the other client that recieved the image and view count isn't online in case it's online the update request should be sent directly Okay so this is admin UI I want another UI for the user to use 
first the user register or logs in using only his or her user name and then he can see his 128 by 128 images that is both is saved locally and on the server and he can upload more to the server 
then in another tab the user can see who is online when the user can expand the view so each user below it is the 128 by 128 images that he uploaded or the user images if collapsed only the clicked user explands then the user can click on an image a click request to view
then the image request is created then in another tab the user can see the request the he have on his images and when he the request is approved the user must give view count and cover image where everything is encrypted and sent to other user
also the recieved request has the name of the 128 by 128 image and maybe the image itself so the client backend can use to match it with the locally saved image to be sent
when a user opens the or loges in it must start a heartbeat then when the UI is closed stop it
then we need another tab for images that the user can click to view
futurely the server will have add notes and get notes that will be used to update the view count incase the user changed his mind however get notes will be called by the client by the server when the user first becomes online add notes will be used in case the client updated the view count but the other client that recieved the image and view count isn't online in case it's online the update request should be sent directly                                  | Request                                | Response                                                              |
|------------|--------|-------------|--------------------------------------------------------------|----------------------------------------|-----------------------------------------------------------------------|
